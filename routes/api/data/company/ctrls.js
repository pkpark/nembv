...
exports.list = (req, res) => {
    //res.send({ success: false, msg: 'list 준비중 입니다.' });
    // Company.find()
...
exports.add = (req, res) => {
    // res.send({ success: false, msg: 'add 준비중 입니다.' });
    ...
    const { name } = req.body;
    if (!name) res.send({ success: false, msg: 'name not exist' });
...
exports.mod = (req, res) => {
    // res.send({ success: false, msg: 'mod 준비중 입니다.' });

    const set = req.body;
    if (!Object.keys(set).length) return res.send({ success: false, msg: 'body not set' });
    if (!set._id) return res.send({ success: false, msg: 'id not exist' });
    set.ut = new Date();
...
exports.del = (req, res) => {
    // res.send({ success: false, msg: 'del 준비중 입니다.' });
